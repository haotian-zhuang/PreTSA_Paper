...
set.seed(2025)
counts <- counts[sample(rownames(counts), 2000), rownames(coord)]

example_sce <- SingleCellExperiment(list(counts = counts),
                                    colData = data.frame(row.names = rownames(coord),
                                                         spatial1 = coord[, "row"],
                                                         spatial2 = coord[, "col"]))
...
ggsave(paste0("simu/", f, "/simdata/plot_de.jpg"), dpi = 500, width = 10, height = 6)
...
long$y = rep(loc[,2],5)
as_tibble(long, rownames = "Cell") %>% ggplot(aes(x = x, y = y, color = Expression)) +geom_point(size = 0.1)+facet_grid(~gene)+ scale_colour_gradientn(colors = viridis_pal(option = "magma")(10), limits=c(0, 1)) + coord_fixed(ratio = 1) + theme(axis.text.x = element_text(angle = 45))
ggsave(paste0("simu/", f, "/simdata/plot_nonde.jpg"), dpi = 500, width = 10, height = 6)
